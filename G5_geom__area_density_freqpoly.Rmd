...
A continuación se resumen las opciones y parámetros más importantes de **geom_area**:
...
EL **kernel** es un tipo especial de función de probabilidad de densidad que tiene ciertas propiedades específicas, ya sea que esta no sea negativa y con valores reales de manera tal que el gráfico salga simétrico, y que la suma de la integral sea igual a uno.
...
a=ggplot(DW, aes(Distance))
a+geom_density(kernel = c(kernel="gaussian"),
               alpha=0.4, fill="blue")+
...
A continuación se resumen las opciones y parámetros más importantes de **geom_density**:
...
## Gráfico de frecuencia de polígono con geom\_freqpoly

El gráfico de frecuencia de polígono es similar al gráfico de área y de densidad.  También se puede cambiar la cantidad de compartimentos usando {binwidth}.  En el gráfico de polígono, es solamente la línea la que graficamos y no hay parámetro de {fill} del área debajo de la línea como se muestra en la Figura \ref{fig:dipodium-poligono} a continuación..

```{r dipodium-poligono, echo=TRUE, tidy=FALSE,message=FALSE, warning=FALSE,  fig.align='center', fig.height=4, fig.width=5}
...
En un gráfico similar, Figura \ref{fig:dipodium-poligono2}, se modifican otras opciones como sigue: intensidad del color con \emph{alpha}, el tipo de línea con \emph{linetype} y el grosor de la línea con \emph{size} tal como se muestra a continuación.

```{r dipodium-poligono2, echo=TRUE, tidy=FALSE,message=FALSE, warning=FALSE,  fig.align='center', fig.height=4, fig.width=5}



a=ggplot(DW, aes(Distance, colour=Species_Name))
a+geom_freqpoly( alpha=0.8, size=1.5, binwidth=.1)+
  labs(x="Distancia", y="Frecuencia")+
  theme(axis.title=element_text(size=14,face="bold"))

```

A continuación se resumen las opciones y parámetros más importantes de geom\_freqpoly:

\begin{itemize}
ggplot}(el archivo de datos, aes(la variable continua))
   \begin{itemize}
geom\_freqpoly}(stat={bin}), x, y, alpha, color, fill, linetype, size
   \begin{itemize}
  alpha}: la intensidad del color
  color}: el color de la línea alrededor del área
  linetype}: representa el estilo de línea; vea sección \hyperref[sec:Especificación Estética]{{Especificación Estética}}
  size}: representa el grosor de la línea
...
DW %>%
  select(DBH, Species_Name) %>%
   filter(Species_Name=="r")%>%
  summarize(promedio=mean(DBH, na.rm=TRUE))
