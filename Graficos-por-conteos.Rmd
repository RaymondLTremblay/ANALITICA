...
# Gráficos para conteos por localidad con **geom_point** y **geom_count**
...
En este gráfico (Figura ref{fig:geom point-count}) tenemos la peculiaridad que hay múltiples lagartos con el mismo peso y tamaño, por lo que los puntos quedarán solapados.  A consecuencia de esto, uno no necesariamente va a poder apreciar la frecuencia de los datos.  Más adelante se demostrará cómo resolver ese problema.

Por otro lado, la función geom_count varía el tamaño de los puntos de acuerdo a como cambia el tamaño con relación a una variable. Esta función es una extensión de geom_point.  Con geom_count veremos que la diferencia es que ahora el tamaño de los puntos está relacionado a su frecuencia en el archivo de datos. El tamaño de cada punto representa valores que cuando son pequeños es que son menos frecuentes en el archivo de datos y los más grandes es que son más comunes.  Esto se demuestra en la Figura ref{fig:geom count1} a continuación:
...
En el gráfico de la Figura  también los tamaños de los puntos están proporcionales a su frecuencia. Note que hay que añadir size=..prop.. y group=1 para que trabaje bien la función. Note que en este caso la suma de todas las frecuencias es 1.  Vemos que, al contrario de la Figura }, ahora los puntos salen en proporción a la frecuencia.
...
df=data.frame(age,kids)
df
```

```{r}
ggplot(df,aes(age, kids))+
  geom_count(aes(size=..prop.., group=1))+
...
En el siguiente gráfico, Figura , una de las variables, WEIGHT o peso del lagartijo, se usa para que la suma de la proporción equivalga a 1. *Entonces aquí cada grupo de {Y} suma a 1*. Por ejemplo, en los *Anolis* de peso de 2 gramos hay 4 puntos de valor de 0.25 que da una suma de 1.
...
Si hay más de un grupo, se puede ver la proporción basada en los grupos usando colour. Aquí, en la Figura , vemos el efecto de usar el color, y del patrón uno observa que la mayoría de los pequeños *Anolis* se observa en el periodo lluvioso y los grandes en el periodo seco.
