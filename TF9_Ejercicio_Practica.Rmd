...
```
 ...
 + Hacer gráfico de la variable de respuesta. ¿Cual de las variables de las listas es la variable de respuesta parta una regesión logística y porque?
...
Evaluar un modelo para tratar de predecir la cantidad de ciclista que pasan por uno d elos puentes de New York
...
+ CREATE A COEFFICIENT PLOT OF THE MODEL
...
Evaluate if the propotion of invasive plants is correlated with the 
 ...
 In this chunk, I calculate the proportion of plants which are alien based of the total flora of the island, Proportion invasive = Prop_inv
...
A second example: This data comes from the Philippines and is an survey of the amount economic status of 41544 families.  The survey includes 60 variables.  To see the names of the variables write ina chunk **names(Phili)**
...
Calculate the proportion of the families cost of food per family based on the families income.  

```{r}
Phili$PropFood=Phili$total_food_expenditure/Phili$total_household_income
```

Proozca un grafico de la variable de respuesta **PropFood**.  Evaluar el porcentaje del income que se usa para comida.   Cumple con las condiciones de una beta?  Si no como se resuelve?
