...
Si el gráfico de residuos de la regresión lineal indica una distribución no aleatoria del error de predicción del modelo lineal. Es posible que pueda mejorar su predicción utilizando una regresión con un término cuadrático (es decir, X2), por lo que ahora tenemos una **regresión con un polinomio.**
...
Nota que la linea `r colorize("azul", "blue")` representa el modelo de regresión lineal y la linea `r colorize("roja", "red")` representa el modelo $x^2$. Es claro que el modelo $x^2$ es un mejor modelo ya que los datos se acerca más a la linea.
...
Recuerda que los residuales tienen que ser normalmente distribuido a través de la variables de **x**. En este caso vemos que los residuales no están concentrado en un área de la gráfico. Por ejemplo los residuales no son más grande en los valores de **x** más grande.
...
Ahora vemos como se ve los modelos de `r colorize("regresión lineal", "blue")` y `r colorize("cuadratico", "orange")`. nota que el cubico los datos siguen mucho mejor los datos.
...
La cantidad de variación contabilizada toma en cuenta 84.6%. La significación del término cuadrático (t = 13.31, p \<0.001) sugiere que la inclusión del término cubica mejora el ajuste del modelo. El error de predicción (residuales) es menor y su distribución parece más aleatoria.
...
Aquí se demuestra algunos de los patrones que pudiese sugerir que los datos no cumple con los supuestos de la prueba. El grafico es de Field, Miles and Field del libro Discovering Statistics Using R, 2012. La figure 7.15 (Con permiso del autor A. Field, email, 2020-10-08) .
